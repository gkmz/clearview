...
- 提醒操作：立即完成、稍后提醒、重启计时。
- 支持护眼与番茄两种节奏：护眼按固定间隔提醒远眺，番茄按专注/短休息周期运行，完成一组专注后进入长休息，并在休息开始时引导舒眼。
- 蓝光过滤档位：`原色`、`轻柔`、`舒缓`、`夜间`。
- 背景图片支持跟随系统、按时间切换或固定白天/夜晚；背景切换不会自动改变蓝光过滤。
- 支持分别自定义浅色和深色背景图片；图片会复制到应用目录并自动等比缩放，最长边限制为 4096 像素，低分辨率图片允许使用但会提示可能模糊。
- 支持多个全局快捷键自定义。
...
在「节奏」设置中可以选择：
- **护眼**：按设定间隔提醒你远眺休息。
- **番茄**：按专注/短休息周期运行。默认每组 4 轮，也可以配置为 2 或 3 轮；完成一组后进入默认 15 分钟长休息。每次休息开始时先引导舒眼。

在「偏好」中，你可以开启开机启动，也可以决定应用启动后是否自动开始计时。
...
> 构建策略：当前版本仅支持通过 Xcode / `xcodebuild` 构建，以确保产出完整的 macOS `.app` 包（图标资源、Bundle 元信息与打包流程）。
